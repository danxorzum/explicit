...
''';
        ...
        expect(result.violations.first.rule, PublishSafety.noForcePublishRule);
      });
...
''';
        ...
        expect(result.isSafe, isFalse);
        expect(result.violations, hasLength(1));
        expect(result.violations.first.rule, PublishSafety.noMelosNoDryRunRule);
      });
...
''';
        ...
        expect(result.isSafe, isFalse);
        expect(result.violations, hasLength(1));
        expect(result.violations.first.rule, PublishSafety.noTokenEnvRule);
      });
...
''';
        ...
        expect(result.isSafe, isFalse);
        expect(result.violations, hasLength(1));
        expect(result.violations.first.rule, PublishSafety.noOidcRule);
      });
...
''';
    ...
    group('assertSafeContent with workflow/job allowlist', () {
      test('allows id-token: write in publish.yaml::publish_package', () {
        const content = '''
permissions:
  id-token: write
  contents: read
...
''';
      ...
      test('credentials (PUB_TOKEN) forbidden everywhere including '
          'publish jobs', () {
        const content = '''
env:
  PUB_TOKEN: secret
''';
      ...
      test('credentials (PUB_CREDENTIALS) forbidden in publish jobs', () {
        const content = '''
env:
...
''';
      ...
      test('denies plain dart pub publish in non-approved job '
...
''';
      ...
      test('denies plain dart pub publish in release_version_pr.yaml', () {
        const content = '''
      - name: Publish
        run: dart pub publish
''';
...
''';
      ...
      test('denies plain dart pub publish with no workflow/job context', () {
        const content = '''
run: dart pub publish
''';
      ...
      test('denies plain dart pub publish in unknown workflow', () {
        const content = '''
run: dart pub publish
''';
        final result = PublishSafety.assertSafeContent(
...
''';
      ...
      test('dry-run publish not flagged by plain publish rule', () {
        const content = '''
    ...
    group('SafetyResult', () {
      ...
      test('isSafe is false when violations exist', () {
        const result = SafetyResult(
          violations: [
            SafetyViolation(rule: 'test', line: 1, matchedText: 'bad'),
          ],
        );
        expect(result.isSafe, isFalse);
      });
